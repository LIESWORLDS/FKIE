...
log = logging.getLogger("ALRICESBOT")

DEFAULT_SETTINGS = {
    "bot_name": "ALRICES BOT",
    "referral_reward": "20",
    "daily_bonus": "25",
    "premium_daily_bonus_multiplier": "2",
    "premium_daily_bonus": "40",
    "support_username": "ALRICES",
    "payment_username": "ALRICES",
    "force_join": "1",
    "maintenance": "0",
    "star_rate": "30",
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    admin = await is_admin(user.id)
    bot_name = await get_setting("bot_name", "IMMORTAL FILE STORE")
...
async def handle_support(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await guard_user(update, context):
        return
    support_username = await get_setting("support_username", "IMMORTALDADY")
    text = (
        "💬 <b>Support</b>\n\n"
...
async def cb_buycoins_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    payment_username = await get_setting("payment_username", "IMMORTALDADY")
